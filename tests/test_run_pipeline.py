...
class TestProcessScene:
    ...
    def test_2d_calls_segment_2d(
        ...
        mock_img_2d, mock_models, tmp_path
    ):
        ...
        mocker.patch(
            ...
            return_value=(np.array([[7,7]]), SimpleNamespace(flow=np.zeros((5, 5, 3))))
        )
        mocker.patch("spot_detector.run_pipeline.make_qc_figure")
        
        result = _process_scene(
            img=mock_img_2d, config=base_config, do_3d=False, models=mock_models,
            mode="2d", condition="Control", filepath=Path("test.nd2"),
            experiment="exp1", scene=0, fig_dir=tmp_path
        )
    ...
    def test_3d_calls_segment_3d_with_stitch_threshold(
        ...
        mocker.patch(
            "spot_detector.run_pipeline.detect_spots_spotiflow",
            return_value=(np.array([[2,7,7]]), SimpleNamespace(flow=np.zeros((5, 5, 4))))
        )
        mocker.patch("spot_detector.run_pipeline.make_qc_figure")
        
        result = _process_scene(
            img=img, config=base_config, do_3d=True, models=mock_models,
            mode="3d", condition="Treated", filepath=Path("test.nd2"),
            experiment="exp1", scene=2, fig_dir=tmp_path
        )
    ...
    def test_qc_figure_called_with_correct_path_and_condition(
        ...
        mocker.patch(
            "spot_detector.run_pipeline.segment_2d", return_value=np.zeros((20, 20), dtype=int)
        )
        mocker.patch(
            "spot_detector.run_pipeline.detect_spots_spotiflow",
            return_value=(np.array([[7,7]]), SimpleNamespace(flow=np.zeros((5, 5, 3))))
        ...
        _process_scene(
            img=mock_img_2d, config=base_config, do_3d=False, models=mock_models,
            mode="2d", condition="Control", filepath=Path("test.nd2"),
            experiment="exp1", scene=3, fig_dir=tmp_path
        )
    ...
    def test_passes_filepath_name_to_measure_objects(
        ...
        _process_scene(
            img=mock_img_2d, config=base_config, do_3d=False, models=mock_models,
            mode="2d", condition="Control", filepath=Path("/some/dir/Control_01.nd2"),
            experiment="exp1", scene=3, fig_dir=tmp_path
        )
...
class TestProcessFile:
    ...
    def test_combines_multiple_scenes(
        self, mocker: MockerFixture, base_config, 
        mock_models, tmp_path
    ):
        mock_img_instance = mocker.MagicMock()
        mock_img_instance.scenes = ["scene0", "scene1"]
        mocker.patch("spot_detector.run_pipeline.BioImage", return_value=mock_img_instance)
        ...
        mock_summary = mocker.patch("spot_detector.run_pipeline.make_scene_summary_figure")
        
        result = _process_file(
            filepath=Path("Control_01.nd2"), config=base_config, models=mock_models,
            mode="2d", do_3d=False, experiment="exp1",
            fig_dir=tmp_path, tab_dir=tmp_path
        )
    ...
    def test_derives_condition_from_filename(
        ...
        _process_file(
            filepath=Path("Treated-DrugA_FOV3.nd2"), config=base_config, models=mock_models,
            mode="2d", do_3d=False, experiment="exp1",
            fig_dir=tmp_path, tab_dir=tmp_path,
        )
...
class TestRunPipeline:
    ...
    def test_combines_multiple_files_and_derives_experiment_name(self, mocker: MockerFixture, tmp_path):
        ...
        mocker.patch("spot_detector.run_pipeline.ModelBundle.load", return_value=mocker.MagicMock())
        
        df_a = pd.DataFrame({"Object_Label": [1], "Condition": ["Control"]})
        df_b = pd.DataFrame({"Object_Label": [1], "Condition": ["Treated"]})
        mocker.patch("spot_detector.run_pipeline._process_file", side_effect=[df_a, df_b])
        mock_run_summary = mocker.patch("spot_detector.run_pipeline.make_run_summary_figure")
    ...
    def test_returns_none_with_no_files_processed(self, mocker: MockerFixture, tmp_path):
        ...
        config = {
            "mode": {"do_3d": False},
            "paths": {"raw_data_dir": str(data_dir), "out_dir": str(project_root / "output")},
        }
        mocker.patch("spot_detector.run_pipeline.ModelBundle.load", return_value=mocker.MagicMock())
