...
def run_pipeline(config: dict) -> pd.DataFrame | None:
    ...
    make_run_summary_figure(
        ...
        out_path=fig_dir / f"_run_summary_{mode}.png"
    )
    return run_df    
...
def _process_file(
    ...
    for scene in tqdm(range(num_scenes)):
        ...
        scene_df = _process_scene(
            ...
            experiment=experiment, fig_dir=fig_dir
        )
        if scene_df is not None:
...
def _process_scene(
    ...
    objects_stack = img.get_image_data(dim_order, C=config["channels"]["segmentation_image"]).astype(np.float32)
    spots_stack = img.get_image_data(dim_order, C=config["channels"]["spot_image"]).astype(np.float32)
    ...
    make_qc_figure(
        condition=condition, scene=scene, mode=mode,
        out_path=fig_dir / f"{condition}_S{scene:02d}_{mode}_qc.png",
        segmentation_image=objects_stack, spots_image=spots_stack,
        masks=masks, coordinates=points, spot_labels=spot_labels,
        flow_details=details, dx=dx, dz=dz, config=config
    )
    return scene_df